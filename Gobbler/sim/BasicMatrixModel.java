...
public abstract class BasicMatrixModel implements MatrixModel {
	 ...
	 * by the BasicMatrixModel constructor during initialization. It can use
	 * any technique it likes to decide what types of Critters to add, where
	 * to put them, etc.  The returned array must have  the dimensions given,
	 * and contain Critter objects and nulls.
	 *
		...
		for(int i = 0; i < rowCount; i++) {
			for(int j = 0; j < colCount; j++) {
				if(cell[i][j][bFrom] != null) {
					...
					if((bug.getCritterInfo().getAge() % bug.getCritterInfo()
															   .getReproductionSpan()) == 0) {
						...
						if(newLoc != null) {
							...
							while(iter.hasNext()) {
								((MatrixView) iter.next()).addCritter(littleBug,
																	  newLoc);
							}
						}
					}
	...
	/**
	 * Select the fittest survivor from the list of Critters that would like to
	 * inhabit this cell.
	 *
	...
	/**
	 * Find a nearby empty cell in one of the cell buffers.  Returns null if no
	 * empty cells found.  Picks randomly from the empty candidates.
	 *
	 * @param row
	 * @param col
	 * @param bSelect
	 ...
	 * factoids about a Critter.  In addition to the parameters supplied here,
	 * the CritterInfo object maintains dynamic values such as age and current
	 * calories.
	 * 
	 * <p>
	 * This factory method is used instead of a constructor so that a Critter
	 * can create only one CritterInfo object to store information about
	 * itself.
	 * </p>
	 *
	 ...
	 * @param burn the number of calories burned each tick of the clock.
	 * 		  minimum is 1.
	 * @param strength the maximum strength of this Critter in arbitrary units
	 */
	public static CritterInfo createCritterInfoInstance(Critter me, int life,
														int repro, int maxCal,
														int burn, int strength) {
	...
	/**
	 * This is the matching method to createCritterInfoInstance.  It removes  a
	 * CritterInfoContainer from the Map when we are done with it.
	 *
	 ...
	 * fashion.  The purpose of this class is to provide read-only access to
	 * Critter information for the Critters, while also providing read/write
	 * access to this MatrixModel.
	 *
	 * @see CritterInfo
	 */
	private static class CritterInfoContainer implements CritterInfo {
		 ...
		 * @param burn the number of calories burned each tick of the clock.
		 * 		  minimum is 1.
		 * @param strength the strength of this Critter, in arbitrary units.
		 * 		  minimum is 1.
		 */
		private CritterInfoContainer(int life, int repro, int maxCal, int burn,
									 int strength) {
			age = 0;
			lifeSpan = life;
		...
		public int getPower() {
			if(age == 0) {
				...
				return Math.max(1,
								(int) (strength * ((double) curCalories / (double) maxCalories)));
			}
		}
		...
		/**
		 * Set the current calories value of this Critter.  If the given value
		 * is outside the 0..maxCalories then curCalories is set to the
		 * associated limit.
		 *
		...
		/**
		 * Increment the current calories value of this Critter by the delta
		 * given.  Delta can be pos or neg.  If the resulting value is outside
		 * the 0..maxCalories then curCalories is set to the associated limit.
		 *
